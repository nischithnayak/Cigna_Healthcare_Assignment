...
public class Main {
    ...
    public static boolean check(String id) {
        ...
        String first3 = id.substring(0, 3);
        String nextThree = id.substring(3, 6);
        String time = id.substring(6, 8);
        String timeZone = id.substring(8, 10);

        if (!first3.equalsIgnoreCase("SPC")) {
            return false;
        }

        if (!nextThree.matches("\\d{3}")) {
            return false;
        }
        ...
        if (timeInt < 1 || timeInt > 12) {
            return false;
        }

        if (!timeZone.equalsIgnoreCase("AM") && !timeZone.equalsIgnoreCase("PM")) {
            return false;
        }
